...
class NNCS_NonLinear:
    ...
    def setPlant(self,dim,nI,dynamics_func,Ts, outputMat,feedbackMap,reachableSteps):
        self.dim =  dim
        self.nI = nI
    ...
    def invokeReachibility(self):
        ...
        result = self.eng.NonLinearNNCS_reach(self.nnfile,self.dynamics_func,self.dim,self.nI,self.Ts,self.reachableSteps,self.outputMat,self.feedbackMap, self.lb,self.ub,self.steps,self.reach_method,self.cores,self.lbRefInput,self.ubRefInput, nargout = 2)
        R,rt =  result
        print(rt)
        return result
        # return self.eng.NonLinearNNCS_reach(self.nnfile,self.dynamics_func,self.dim,self.nI,self.Ts,self.reachableSteps,self.outputMat,self.feedbackMap, self.lb,self.ub,self.steps,self.reach_method,self.cores,self.lbRefInput,self.ubRefInput)
